...
// Brilho que tinge o card conforme a direção do arraste: verde (curtir, vindo da
// direita) e vermelho (passar, vindo da esquerda).
const TINT_LIKE = 'linear-gradient(270deg, rgba(25,182,163,.7), rgba(25,182,163,0) 62%)';
const TINT_NOPE = 'linear-gradient(90deg, rgba(232,71,58,.7), rgba(232,71,58,0) 62%)';
...
const PetCard = forwardRef<PetCardHandle, PetCardProps>(function PetCard(
  ...
  // Brilho colorido (verde/vermelho) sobreposto à foto durante o arraste.
  ...
  // Carimbo visível durante o arraste: -1 = "Passar", +1 = "Curti", 0 = nenhum.
  ...
  function leave(dir: SwipeDir) {
    ...
    // Feedback de saída: tinge o card com a cor da direção e toca o som.
    const tint = tintRef.current;
    if (tint) {
      tint.style.transition = 'opacity .2s ease';
      tint.style.background = dir === 'like' ? TINT_LIKE : TINT_NOPE;
  ...
  /** Atualiza o brilho colorido conforme o deslocamento horizontal do arraste. */
  function paintTint(dx: number) {
    const tint = tintRef.current;
    if (!tint) return;
    const strength = Math.min(Math.abs(dx) / 140, 0.8);
    if (dx > 8) {
      tint.style.background = TINT_LIKE;
      tint.style.opacity = String(strength);
    } else if (dx < -8) {
      tint.style.background = TINT_NOPE;
      tint.style.opacity = String(strength);
    } else {
  ...
  function onPointerDown(e: PointerEvent<HTMLDivElement>) {
    ...
    if (tintRef.current) tintRef.current.style.transition = 'opacity .08s linear';
  ...
  function onPointerUp() {
    ...
    if (Math.abs(dx) > 110) {
      ...
      if (tintRef.current) {
        tintRef.current.style.transition = 'opacity .2s ease';
  ...
  return (
    ...
    >
      <div className="photo" style={{ background: `linear-gradient(150deg, ${c1}, ${c2})` }}>
        ...
        <div className="scrim" />
        <div className="swipe-tint" ref={tintRef} />
        ...
        <div className="info">
          {typeof pet.score === 'number' && (
            <div className="score-pill">
              <Heart className="ic-inline" aria-hidden /> {scorePercent(pet.score)}% de match
            </div>
          )}
          <h2>
          ...
          <div className="meta">
            <MapPin className="ic-inline" aria-hidden /> {pet.city ?? 'Brasil'} • {speciesLine(pet)}
          </div>
          {reasons.length > 0 && (
            <div className="reasons">
              <Sparkles className="ic-inline" aria-hidden /> {reasons.join(' · ')}
            </div>
          )}
